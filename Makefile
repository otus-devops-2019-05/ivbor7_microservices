...
# User name used also to login Docker registry
...
$(MON_SERVICES):
	@echo "$(date +"%Y-%m-%d %H:%M:%S") build $(DOCKER_REPO)/$@:$$VER" >> make_info.txt
...
$(REDDIT_IMAGES):
	@echo "$(date +"%Y-%m-%d %H:%M:%S") push $(DOCKER_REPO)/$(subst img_,,$@):$$VER" >> make_info.txt
